...
- **信使图片模式开关**（新功能）：
  - **开启**：当AI消息中包含 `<pic prompt="...">` 标签时，自动生成并显示图片
  - **关闭**：去除 `<pic>` 标签，仅以纯文字形式呈现（例如，AI回复"发送了一张便当照片"时，只显示文字描述，不生成图片）
- 扩展菜单中简单的开关
- 扩展设置面板中的配置选项
- 自定义提示词模板和正则
...
2. 在扩展设置面板中配置图像插入类型
3. 使用 **信使图片模式** 开关控制图像生成行为：
   - **开启**：当AI在消息中包含 `<pic prompt="...">` 时，扩展将自动生成图像
   - **关闭**：`<pic>` 标签将被去除，仅显示AI回复的文字内容
